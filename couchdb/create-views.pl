...
function(doc) {
    if( doc.type === 'post' && doc.post_status === 'public' ) {
        emit(doc.updated_at, {slug: doc._id, title: doc.title, text_intro: doc.text_intro, more_text_exists: doc.more_text_exists, tags: doc.tags, post_type: doc.post_type, author: doc.author, updated_at: doc.updated_at, reading_time: doc.reading_time});
    }
}
VIEWJS1
...
VIEWJS2
$perl_hash->{'views'}->{'post_html'}->{'map'} = $view_js;
