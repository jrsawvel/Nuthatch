...
function M.retrieve_markup(post_id)

    local db = config.get_value_for("database_name")

    local url = 'http://127.0.0.1:5984/' .. db .. '/_design/views/_view/post_markup?key="' .. post_id .. '"'
    ...
    if post.markup == nil then
        rj.report_error("400", "Unable to retrieve " .. post_id .. ".", "Content does not exist." .. url)
        return nil
    else 
        return post.markup
    end
...
function M.read_markup_file(post_id)
    ...
    if f == nil then
        rj.report_error("400", "Could not open " .. post_id .. ".txt for read.", "File may not exist.")
    else
        for line in f:lines() do
            markup = markup .. line .. "\n"
        end
